...
export function rewriteConnectFrame(raw, gatewayAuth) {
  ...
  if (frame && frame.type === 'req' && frame.method === 'connect' && frame.params && typeof frame.params === 'object') {
    ...
    // Inject server-side gateway auth (token preferred over password).
    // Device auth for the browser proxy requires challenge nonce capture in serve.js;
    // the internal server-side client (gateway-client.js) uses device auth instead.
    const authObj = (gatewayAuth && typeof gatewayAuth === 'object')
      ? gatewayAuth
      : (gatewayAuth ? { token: gatewayAuth } : {});
    if (authObj.token) {
      p.auth = { token: authObj.token };
    } else if (authObj.password) {
      p.auth = { password: authObj.password };
    } else if (!p.auth) {
      p.auth = {};
    }
    // Ensure operator.admin scope is requested
