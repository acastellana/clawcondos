...
export function rewriteConnectFrame(raw, gatewayAuth) {
  ...
  if (frame && frame.type === 'req' && frame.method === 'connect' && frame.params && typeof frame.params === 'object') {
    ...
    // Always enforce server-side auth to avoid stale browser tokens causing gateway mismatch.
    // Prefer password when available (works with gateway.auth.mode=password),
    // otherwise fall back to token.
    if (authObj.password) {
