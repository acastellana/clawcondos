...
/** Get the paired device operator token for operator.admin scope. */
...
export function rewriteConnectFrame(raw, gatewayAuth) {
  ...
  if (frame && frame.type === 'req' && frame.method === 'connect' && frame.params && typeof frame.params === 'object') {
    ...
    // Prefer device token (grants operator.admin/read scopes via paired device identity).
    const deviceToken = _getDeviceToken();
    if (deviceToken) {
      p.auth = { deviceToken };
    } else {
      // Fallback: inject server gateway token
      const authObj = (gatewayAuth && typeof gatewayAuth === 'object')
        ? gatewayAuth
        : (gatewayAuth ? { token: gatewayAuth } : {});
      if (authObj.password) {
