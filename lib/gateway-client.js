...
/**
 * Build a signed device auth payload for operator.admin scope.
 * Returns { payload, signature, publicKey } or null if device identity unavailable.
 */
function buildDeviceAuth(scopes = ['operator.admin', 'operator.read']) {
  try {
    ...
    // Public key: SPKI DER bytes, offset 12 = raw Ed25519 key bytes, base64url
        ...
        ws.on('message', (raw) => {
          ...
          if (state === 'authenticating') {
            if (msg.type === 'event' && msg.event === 'connect.challenge') {
              ...
              const deviceAuth = buildDeviceAuth(['operator.admin', 'operator.read']);
              const connectFrame = {
                type: 'req',
                id: _nextId(),
                method: 'connect',
                params: {
                  ...
                  auth: deviceAuth ? { device: deviceAuth } : auth
                }
              };
