...
/**
 * Get the paired device operator token from ~/.openclaw/identity/device-auth.json.
 * This token grants operator.admin + operator.read scopes when used as auth.deviceToken.
 */
function getDeviceToken() {
  try {
    const authFile = join(homedir(), '.openclaw', 'identity', 'device-auth.json');
        ...
        ws.on('message', (raw) => {
          ...
          if (state === 'authenticating') {
            if (msg.type === 'event' && msg.event === 'connect.challenge') {
              ...
              // Prefer device token (grants operator.admin/read); fall back to gateway token/password.
              ...
              const connectFrame = {
                ...
                params: {
                  ...
                  auth: deviceToken ? { deviceToken } : auth
                }
              };
