 ...
 */
function buildDeviceConnectParams(scopes = ['operator.admin', 'operator.read']) {
  try {
    ...
    // Build v2 payload string (same format as OpenClaw CLI)
    ...
    return {
      auth: operatorToken ? { token: operatorToken } : {},
        ...
        ws.on('message', (raw) => {
          ...
          if (state === 'authenticating') {
            if (msg.type === 'event' && msg.event === 'connect.challenge') {
              ...
              const devParams = buildDeviceConnectParams(['operator.admin', 'operator.read']);
              const connectFrame = {
                type: 'req',
                id: _nextId(),
                method: 'connect',
